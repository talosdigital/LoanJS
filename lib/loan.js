...
(function() {
  ...
  moment.fn.findNextWorkingDate = function() {
    var nextDay = moment(this).add(1, 'days');
 ...
 *                  }
 */
var Loan = function (amount, installmentsNumber, interestRate, diminishing, periodDuration, periodType) {
  'use strict';
  if(!amount || amount <= 0 ||
...
var LoanScheduleDates = {
  ...
  getAllPaymentsDates: function (periodDuration, periodType, numberOfInstallments){
    ...
    for (periodIterator=0; periodIterator<numberOfInstallments; periodIterator++){
      ...
      var nextPaymentDate = moment(currentDate).add(periodDuration, periodType);

      if (nextPaymentDate.isWorkingDay()){

        rawPaymentDates[periodIterator] = nextPaymentDate;
        // baseline date
        currentDate.add(periodDuration, periodType);
        ...
        rawPaymentDates[periodIterator] = moment(nextWorkingDay);
        // baseline date
